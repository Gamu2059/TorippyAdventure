...
int motionChange(int type){
	...
	switch(type){
		case 0:
			if(count>=0&&count<15)motion=getPlayerMoveNumber(&dummy,WALK);
			else if(count>=15&&count<30)motion=getPlayerMoveNumber(&dummy,WALK)+1;
			else if(count>=30&&count<45)motion=getPlayerMoveNumber(&dummy,WALK)+2;
			else if(count>=45&&count<60)motion=getPlayerMoveNumber(&dummy,WALK)+1;
			break;
		case 1:
			motion=getPlayerMoveNumber(&dummy,S_ATTACK)+4;
			break;
	}
	count++;
	if(count==60)count=0;
...
void drawAnimation(int level){
	int y,x;
	for(y=0;y<OBJECT_LINES;y++){
		for(x=0;x<OBJECT_COLS;x++){
			if(getColorInfo(playerInfo[level][y][x])<64&&playerInfo[level][y][x]!=200000){
				if(	y+60<ENDING_LINES&&
					y+60>=0&&
					x+200<ENDING_COLS&&
					x+200>=0)
					screen[y+60][x+200]=playerInfo[level][y][x];
			}
		}
	}
}
