...
void changeStage(SD*,SD*,CD*,CD*,ID*,int);
...
void scrollDisplay(SD*,SD*,CD*);
void confirmExistCharacter(SD*,CD*);
void confirmExistItem(SD*,ID*);
void gameover(SD*,SD*,CD*,CD*,ID*);
...
void changeStage(SD* stage,SD* back,CD* player,CD* enemy,ID* item,int stageNumber){
	int i;
	/***** ステージ共通設定 *****/
	/* ステージと背景の生成 */
	createStage(stage,back,stageNumber);
	/* プレイヤーの構成の生成 */
...
void changeEnemy(CD* cd,int stageNumber){
	...
	switch(stageNumber){
		case 0:
			//initializeEnemy(&cd[num],0,NO_ACTION,LEFT);cd[num].y=1;cd[num++].x=40;
...
void changeItem(ID* id,int stageNumber){
	...
	switch(stageNumber){
		case 0:
			/* initializeItem(&id[num],0,NO_ACTION,RIGHT);id[num].y=1;id[num++].x=40; */
	...
	for(y=0;y<CHARACTER_LINES;y++){
		for(x=0;x<CHARACTER_COLS;x++){
			if(
				...
				if(getPriorityInfo(sd->stage[cd->y+y][cd->x+x])<=1)
					cd->back[y][x]=sd->stage[cd->y+y][cd->x+x];
				else
					cd->back[y][x]=9099;
	...
	for(y=0;y<CHARACTER_LINES;y++){
		for(x=0;x<CHARACTER_COLS;x++){
			if(
				...
				(getPriorityInfo(cd->back[y][x])<=1)
			)
				sd->stage[cd->y+y][cd->x+x]=cd->back[y][x];
		}
	}
}
...
void outputStage(SD* sd,SD* bd){
	int y,x,z;
	/* ステージ背景の描画 */
	for(y=bd->displayY;y<DISPLAY_LINES+bd->displayY;y++){
		for(x=bd->displayX;x<DISPLAY_COLS+bd->displayX;x++){
			attron(COLOR_PAIR(getColorInfo(bd->stage[y][x])));
			for(z=0;z<2;z++)
				mvaddch(y-bd->displayY,(x-bd->displayX)*2+z,getGradationInfo(bd->stage[y][x]));
		}
	}
	/* ステージの描画 */
...
void outputStatus(CD* cd,int restLife){
	...
	for(y=0;y<3;y++){
		for(x=0;x<100;x++){
			attron(COLOR_PAIR((x<(cd->chargeGauge*100)/cd->maxChargeGauge)?1:4));
			mvaddch(50+y,(DISPLAY_COLS+2)*2+x,' ');
		}
	}
}
void scrollDisplay(SD* sd,SD* bd,CD* cd){
	/* 上方向にスクロール */
	if(getGravityMove(cd->v0,cd->vA,cd->fallTime)<0&&cd->y<=sd->displayY+SCROLL_LINES){
		...
		bd->displayY+=getGravityMove(cd->v0,cd->vA,cd->fallTime)/2;
		if(sd->displayY<sd->minDisplayY)
			sd->displayY=sd->minDisplayY;
		if(bd->displayY<bd->minDisplayY)
			bd->displayY=bd->minDisplayY;
	}
	/* 下方向にスクロール */
	if(getGravityMove(cd->v0,cd->vA,cd->fallTime)>0&&cd->y+CHARACTER_LINES>=sd->displayY+DISPLAY_LINES-SCROLL_LINES){
		...
		if(bd->displayY+DISPLAY_LINES>bd->maxDisplayY)
			bd->displayY=bd->maxDisplayY-DISPLAY_LINES;
	}
	/* 左方向にスクロール */
	if(cd->x<=sd->displayX+SCROLL_COLS){
		...
		if(bd->displayX<bd->minDisplayX)
			bd->displayX=bd->minDisplayX;
	}
	/* 右方向にスクロール */
	if(cd->x+CHARACTER_COLS>=sd->displayX+DISPLAY_COLS-SCROLL_COLS){
		...
		if(bd->displayX+DISPLAY_COLS>bd->maxDisplayX)
			bd->displayX=bd->maxDisplayX-DISPLAY_COLS;
	}
}
void confirmExistCharacter(SD* sd,CD* cd){
	if(
		/* 底に到達した時は即死 */
		(cd->y+cd->downWallY+2==STAGE_LINES)
	)
		cd->hitPoint=0;
	if(
		/* 死亡状態でNO_ACTIONになった時に存在を削除 */
		(cd->deathFlag==TRUE&&cd->motionFlag==NO_ACTION)
	)
		cd->existFlag=FALSE;
	/* HPが0なら死亡状態とする */
	if(cd->hitPoint<=0)
...
void gameover(SD* sd,SD* bd,CD* pd,CD* ed,ID* id){
	int y,x,z;
	CD *cdTmp;
	ID *idTmp;
	erase();
...
void main(){
	...
	while(restLife>0){
		/* ステージ構成を最初の状態でリセット */
		changeStage(&stage,&stageBack,&player,enemy,item,stageNumber);
		/***** プレイヤーの存在している間 *****/
		while(player.existFlag==TRUE){
			...
			for(i=0;i<MAX_ENEMY+1;i++){
				...
				if(
					...
					/* キャラクタの存在判定 */
					confirmExistCharacter(&stage,cdTmp);
					/* キャラクタの反映 */
			...
			/***** スクロール処理 *****/
			scrollDisplay(&stage,&stageBack,&player);
			/***** 出力処理 *****/
			erase();
			/* ステージの表示 */
			outputStage(&stage,&stageBack);
			/* ステータスの表示 */
	...
	/***** ゲームオーバー処理 *****/
	gameover(&stage,&stageBack,&player,enemy,item);
	/* 待機 */
	timeout(-1);
	getch();
	endwin();
