...
char confirmDamageCharacter(SD*,CD*);
...
char confirmDamageCharacter(SD* sd,CD* cd){
	int y,x;
	for(y=0;y<CHARACTER_LINES;y++){
		for(x=0;x<CHARACTER_COLS;x++){
			if(getHitInfo(cd->look[y][x])==2&&getHitInfo(sd->stage[cd->y+y][cd->x+x])==1)
				return TRUE;
		}
	}
	return FALSE;
...
char confirmCrashItem(SD* sd,ID* id){
	...
	for(y=0;y<ITEM_LINES;y++){
		for(x=0;x<ITEM_COLS;x++){
			if(getColorInfo(id->item[y][x])<64){
				if(	!(id->y+y>=0&&id->y+y<STAGE_LINES&&id->x+x>=0&&id->x+x<STAGE_COLS)||
					(getCrashInfo(id->item[y][x])!=0&&getCrashInfo(sd->stage[id->y+y][id->x+x])!=0)
				)
					return TRUE;
			}
		}
...
void modifyCrashItem(SD* sd,ID* id){
	...
	if(id->y+id->downWallY+1==id->downWallLine){
		...
		if(id->v0==0){
			id->v0=id->a0;
			id->jumpCount=1;
			id->fallTime=getParabolaTopTime(id->v0)+ELAPSED_TIME*5;
		}
	}
