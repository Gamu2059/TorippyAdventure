...
char confirmDamageObject(SD*,OD*,int*,int*);
...
char confirmDamageObject(SD* sd,OD* od,int* damageY,int* damageX){
	int y,x;
	...
	for(y=od->now.existStartY;y<=od->now.existEndY;y++){
		for(x=(y==od->now.existStartY)?od->now.existStartX:0;x<OBJECT_COLS;x++){
			if(getColorInfo(od->state.look[y][x])<64){
				if(getCrashInfo(od->state.look[y][x])==2&&getCrashInfo(sd->look[od->now.y+y][od->now.x+x].stage)==2){
					*crashY=y;
					*crashX=x;
					return TRUE;
				}
...
void getWallLineObject(SD* sd,OD* od){
	...
	for(x=od->now.crashStartX;x<=od->now.crashEndX;x++){
		for(distant=od->now.y+y+1;getCrashInfo(sd->look[distant][od->now.x+x].stage)!=2&&distant<=od->now.y+OBJECT_LINES*1.5&&distant<STAGE_LINES;distant++);
	...
	for(x=od->now.crashStartX;x<=od->now.crashEndX;x++){
		for(distant=od->now.y+y-1;getCrashInfo(sd->look[distant][od->now.x+x].stage)!=2&&distant>=od->now.y-OBJECT_LINES*0.5&&distant>=0;distant--);
	...
	for(y=od->now.crashStartY;y<=od->now.crashEndY;y++){
		for(distant=od->now.x+x+1;getCrashInfo(sd->look[od->now.y+y][distant].stage)!=2&&distant<=od->now.x+OBJECT_COLS*1.5&&distant<STAGE_COLS;distant++);
	...
	for(y=od->now.crashStartY;y<=od->now.crashEndY;y++){
		for(distant=od->now.x+x-1;getCrashInfo(sd->look[od->now.y+y][distant].stage)!=2&&distant>=od->now.x-OBJECT_COLS*0.5&&distant>=0;distant--);
...
void modifyCrashObject(SD* sd,OD* od,int label){
	...
	int tmp;
	int count;
	/* crash関連すべてが0ならば修正を行わない */
	if(od->now.crashStartY==0&&od->now.crashStartX==0&&od->now.crashEndY==0&&od->now.crashEndX==0)
		return;
	...
	while(od->now.x+od->now.crashEndX>=od->now.wallRightLine||od->now.x+od->now.crashEndX>=STAGE_COLS)
		od->now.x--;
	if(od->now.x+od->now.crashEndX+1==od->now.wallRightLine&&od->state.vX!=0){
	...
	while(od->now.x+od->now.crashStartX<=od->now.wallLeftLine||od->now.x+od->now.crashStartX<0)
		od->now.x++;
	if(od->now.x+od->now.crashStartX-1==od->now.wallLeftLine&&od->state.vX!=0){
		if(od->now.direction==LEFT){
			if(od->state.fallTime>getParabolaTopTime(od->state.v0)){
				od->state.wallKickFlag=TRUE;
				od->state.jumpCount=0;
			}
