...
void getWallLineObject(SD* sd,OD* od){
	...
	/* 下方向にチェック */
	minDistant=OBJECT_COLS*0.5;
	maxAbsDistant=od->now.y+OBJECT_LINES*1.5;
	y=od->now.crashEndY;
	for(x=od->now.crashStartX;x<=od->now.crashEndX;x++){
		for(distant=od->now.y+y+1;getCrashInfo(sd->look[distant][od->now.x+x].stage)==0&&distant<=od->now.y+OBJECT_LINES*1.5&&distant<STAGE_LINES;distant++);
	...
	for(x=od->now.crashStartX;x<=od->now.crashEndX;x++){
		for(distant=od->now.y+y-1;getCrashInfo(sd->look[distant][od->now.x+x].stage)==0&&distant>=od->now.y-OBJECT_LINES*0.5&&distant>=0;distant--);
	...
	for(y=od->now.crashStartY;y<=od->now.crashEndY;y++){
		for(distant=od->now.x+x+1;getCrashInfo(sd->look[od->now.y+y][distant].stage)==0&&distant<=od->now.x+OBJECT_COLS*1.5&&distant<STAGE_COLS;distant++);
	...
	for(y=od->now.crashStartY;y<=od->now.crashEndY;y++){
		for(distant=od->now.x+x-1;getCrashInfo(sd->look[od->now.y+y][distant].stage)==0&&distant>=od->now.x-OBJECT_COLS*0.5&&distant>=0;distant--);
